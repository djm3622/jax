...
absl::Status CholeskyUpdateImpl(gpuStream_t stream, void** buffers,
  ...
  int nSize = d.matrix_size;
  if (buffers[2] != buffers[0]) {
    // A copy of the input matrix to be iteratively transformed into the result.
    // If the buffer is donated, no copying is needed.
    JAX_RETURN_IF_ERROR(JAX_AS_STATUS(
        gpuMemcpyAsync(buffers[2], buffers[0], sizeof(double) * nSize * nSize,
                       gpuMemcpyDeviceToDevice, stream)));
  }
  if (buffers[3] != buffers[1]) {
    // Rank-1 update vector is copied to be transformed too.
    JAX_RETURN_IF_ERROR(JAX_AS_STATUS(
        gpuMemcpyAsync(buffers[3], buffers[1], sizeof(double) * nSize,
                       gpuMemcpyDeviceToDevice, stream)));
  }
  LaunchCholeskyUpdateKernel(stream, buffers, {nSize});
