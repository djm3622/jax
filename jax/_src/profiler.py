...
class ProfileSessionRunner:
  ...
  @classmethod
  @contextmanager
  def trace(cls, runner: ProfileSessionRunner | None):
    if (runner is None or runner.current_session is not None
      ...
      try:
        ...
        runner.fdo_profiles.append(
            bytes(xla_client.profiler.get_fdo_profile(xspace))
        )
